...
| # | Spec | Session style | Recommended model | Demo beat |
|---|------|---------------|-------------------|-----------|
| 1 | [`00_launcher.md`](00_launcher.md) | Solo, **must finish first** | Sonnet 4.6 | §2 Scaffold |
| 2 | [`01_hangman.md`](01_hangman.md) | Hand-started, parallel | Sonnet 4.6 | §3 Manual parallel |
| 3 | [`02_tictactoe.md`](02_tictactoe.md) | Hand-started, parallel | Sonnet 4.6 | §3 Manual parallel |
| 4 | [`03_word_scramble.md`](03_word_scramble.md) | Hand-started, parallel | Sonnet 4.6 | §3 Manual parallel |
| 5 | [`10_batch_simple_games.md`](10_batch_simple_games.md) | One coordinator → 3 subagents | Opus 4.7 (coord.) + Sonnet 4.6 (subs) | §4 Automatic parallelization |
...
This mirrors the guidance in the reference VS Code video: "don't work on the
same pieces of the code base all at once."

## Refinement notes

Applied during the post-authoring refinement pass:
...
- Forced `rich`-based but *visually distinct* rendering per game (gallows
  ...
  Word Scramble, a dice-results panel for Dice Roll), so the demo's rapid
  cuts between sessions in §3 and §4 read clearly on camera.
- Scoped every spec to exactly two files and an explicit "do not edit any
  other file" rule, so the §3 claim "they can't conflict" is literally true.
- Made the batch coordinator (spec 10) produce a single PR for all three
  subagents, matching the §4 narration ("one coordinator session").
- Pinned recommended models per spec to match what the manuscript says on
  camera (Sonnet for mechanical / narrow-scope work, Opus for the
  coordinator in plan mode).
